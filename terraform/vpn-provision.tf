...
provisioner "file" {
  content = <<-EOF
    ...
    # 1. Fully disable any stub resolver on port 53
    ...
    # 2. Install WireGuard, dnsmasq & iptables-persistent non-interactively
    ...
    # 4. Open firewall ports for WireGuard & DNS
    ...
    # 7. Override systemd-resolved (if any) for DNS
    ...
    # 8. Generate WireGuard client config
